define([
	...
	"sugar-web/graphics/evaluationpalette",
], function (
	l10n,
	shortcut,
	bus,
	...
	activity.setup = function () {
		...
		var addpalette = new evaluationpalette.EvaluationPalette(
		...
		activity.getXOColor(function (error, colors) {
			...
			var invokerElem = document.querySelector(
				"#activity-palette .palette-invoker"
			);
			...
			var newicon = buddyIcon
				.replace(new RegExp("&fill_color;", "g"), colors.fill)
				.replace(new RegExp("&stroke_color;", "g"), colors.stroke);
			var svg_xml = new XMLSerializer().serializeToString(
				new DOMParser().parseFromString(newicon, "text/xml")
			);
