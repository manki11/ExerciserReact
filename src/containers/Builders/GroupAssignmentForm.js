...
class GroupAssignmentForm extends Component {
	...
	componentDidMount() {
		if (this.props.location.state) {
			const { id, title, questions, scores, times, groups } =
	...
	groupAlreadyUsed = () => {
		...
		questions.forEach((question) => {
			if (
				parseInt(question.correctGroup.split("-").pop(), 10) === groups.length
			) {
				alreadyUsed = true;
			}
		});
		...
		if (this.state.isFormValid) {
			...
			if (currentQuestionNo > noOfQuestions) {
				...
				if (currentQuestionNo === noOfQuestions) {
					...
					const { question, answer, correctGroup } =
	...
	submitExercise = (bool, e) => {
		...
		if (bool)
			this.props.history.push("/play/group", {
				exercise: exercise,
				edit: true,
			});
							...
							if (groups) {
								if (mediaType === MULTIMEDIA.image)
									this.props.showMedia(
										text,
										"img",
										this.setGroupSourceFromImageEditor(groupNo)
									);
	...
	render() {
		...
		let groupOptions = groups.map((group, i) => {
			...
			if (!group.type) {
				question = [
					<button
						className='btn button-answer-options button-text col-md-3'
						key='type-1'
						onClick={() => {
					...
					<button
						className='btn button-answer-options button-image col-md-3'
						key='type-2'
						onClick={() => {
				...
				if (groupType === MULTIMEDIA.text)
					question = (
						<div className='answers'>
							...
							/>
							<button
								className='btn button-choices-edit'
								...
								<img
									...
									onClick={() => {
										showMedia(
											group.data,
											"img",
											this.setGroupSourceFromImageEditor(i)
										);
									}}
									alt='Option'
								/>
			...
			return (
				<div className='col-md-8 group-option-type' key={`groups-${i}`}>
					<label htmlFor={`group-${i}`}>{i + 1}</label>
					{question}
				</div>
			);
		});
		...
		if (errors["groupDel"]) {
			group_delete = (
				<span style={{ color: "red" }}>
					<FormattedMessage id={GROUP_DELETE} />
				</span>
			);
		}
					...
					<div className='row align-items-center justify-content-center'>
						...
						>
							<div>
								<p>
									<strong>
										<FormattedMessage id={GROUP_ASSIGNMENT} />
									</strong>
								</p>
								<hr className='my-3' />
								<div className='col-md-12'>
									<form onSubmit={this.handleNewEvent}>
										...
										<div className='row'>
											<div className='form-group'>
												<label htmlFor='question'>
													{id}. <FormattedMessage id={MATCH_ITEM} />:
												</label>
												<button
										...
										<div className='row'>
											<div className='form-group'>
												<label htmlFor='Correct-Group'>
													<FormattedMessage id={CORRECT_GROUP} />:{" "}
												</label>
												<Select
