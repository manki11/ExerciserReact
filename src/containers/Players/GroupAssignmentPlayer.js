...
class GroupAssignmentPlayer extends Component {
	...
	componentDidMount() {
		if (this.props.location.state) {
			const { id, title, questions, scores, times, groups, userLanguage } =
	...
	render() {
		...
		let groupOptions = groups.map((group, index) => {
			...
			return (
				<div
					className={`group-options col-md-${12 / groups.length}`}
		...
		let question = (
			...
			>
				{(this.state.selected || this.state.submitted) && (
					<div className='marker'></div>
				)}
				<div
					className={`box box-width ${btnClass}`}
					id='on-click'
					onClick={(e) => {
		...
		if (
			...
			document.getElementById("question-drag")
		) {
